...
def test_generate_brief_structure(tmp_path):
    ...
    sessions = [{"thread_id": "thread-123", "title": "Test session", "updated_at": "2026-08-06"}]
...
def test_context_spine_order(tmp_path):
    ...
    content = load_hot_context(connector, sources=["system.md", "invariants.md", "goals.md"])
...
def test_real_llm_smoke(tmp_path):
    import os
    if "OPENAI_API_KEY" not in os.environ and "ANTHROPIC_API_KEY" not in os.environ and "LLM_ARCHITECT" not in os.environ:
