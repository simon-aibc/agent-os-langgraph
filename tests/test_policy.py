...
def test_policy_taxonomy():
    ...
    # Allow
    assert policy.evaluate(ActionProposal(tool="t", side_effect="read")).decision == "allow"
    assert policy.evaluate(ActionProposal(tool="t", side_effect="none")).decision == "allow"
    
    # Require approval
    assert policy.evaluate(ActionProposal(tool="t", side_effect="write")).decision == "require_approval"
    assert policy.evaluate(ActionProposal(tool="t", side_effect="network")).decision == "require_approval"
    assert policy.evaluate(ActionProposal(tool="t", side_effect="communication")).decision == "require_approval"
    
    # Deny
    assert policy.evaluate(ActionProposal(tool="t", side_effect="payment")).decision == "deny"
    assert policy.evaluate(ActionProposal(tool="t", side_effect="privileged")).decision == "deny"

def test_policy_logs_briefs_allow():
    policy = LocalPolicy()
    
    # Logs append
    prop_log = ActionProposal(tool="memory.write", side_effect="write", arguments={"ref": "AI/Logs/2026-08-08.md", "mode": "append"})
    assert policy.evaluate(prop_log).decision == "allow"
    assert policy.evaluate(prop_log).policy_id == "builtin-logs"
    
    # Briefs create
    prop_brief = ActionProposal(tool="memory.write", side_effect="write", arguments={"ref": "AI/Briefs/x.md", "mode": "create"})
    assert policy.evaluate(prop_brief).decision == "allow"
    
    # Briefs append
    prop_brief2 = ActionProposal(tool="memory.write", side_effect="write", arguments={"ref": "agentos/briefs/y.md", "mode": "append"})
    assert policy.evaluate(prop_brief2).decision == "allow"
    
    # Not allow (overwrite)
    prop_overwrite = ActionProposal(tool="memory.write", side_effect="write", arguments={"ref": "AI/Logs/2026-08-08.md", "mode": "overwrite"})
