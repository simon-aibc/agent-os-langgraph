...
async def test_chat_multi_turn():
    ...
    with patch("agent_os.cli.app.EventFormatter"):
        with patch("agent_os.bindings.resolve_backend_binding", return_value=BackendBinding(
            router="r", architect="a", executor="e", profile_name="p", sandbox_root="s"
        )):
...
async def test_chat_resume_after_kill():
    ...
    snapshot.values = {"backend_binding": {"router": "r", "architect": "a", "executor": "e", "profile_name": "p", "sandbox_root": "s"}}
...
def test_conversation_summary_default_on_old_checkpoint():
    ...
    assert "conversation_summary" not in loaded or loaded["conversation_summary"] is None
