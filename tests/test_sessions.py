...
def mock_db(tmp_path):
    with patch("agent_os.sessions._get_db_path", return_value=str(tmp_path / "test.db")):
...
async def test_sessions_delete_confirmed(mock_db):
    ...
    with patch("agent_os.cli.app.EventFormatter"):
        with patch("agent_os.cli.app.input", return_value="y"):
            with patch("langgraph.checkpoint.sqlite.aio.AsyncSqliteSaver.adelete_thread", new_callable=AsyncMock) as mock_del:
